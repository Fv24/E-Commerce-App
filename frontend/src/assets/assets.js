import logo from './logo.png';
import search from './search.png';
import profile from './profile.png';
import shoppingcart from './shopping-cart.png';
import menu from './menu.png';
import back from './back.png';
import hero from './hero.jpg';
import image1 from './image1.jpg';
import image2 from './image2.jpg';
import image3 from './image3.jpg';
import image4 from './image4.jpeg';
import image5 from './image5.jpg';
import image6 from './image6.jpg';
import image7 from './image7.jpg';
import image8 from './image8.jpg';
import image9 from './image9.jpeg';
import image10 from './image10.jpg';
import image11 from './image11.jpeg';
import image12 from './image12.jpg';
import image14 from './image14.jpg';
import image15 from './image15.jpg';
import image16 from './image16.jpg';
import image17 from './image17.jpg';
import image18 from './image18.jpg';
import image19 from './image19.jpg';
import image20 from './image20.jpg';
import image21 from './image21.jpg';
import image22 from './image22.jpg';
import image23 from './image23.jpg';
import image24 from './image24.jpg';
import image25 from './image25.jpg';
import image26 from './image26.jpg';
import image27 from './image27.jpg';
import image28 from './image28.jpg';
import image29 from './image29.jpg';
import image30 from './image30.jpg';
import image31 from './image31.jpg';
import image32 from './image32.jpg';
import image33 from './image33.jpg';
import image34 from './image34.jpg';
import image35 from './image35.jpg';
import image36 from './image36.jpg';
import image37 from './image37.jpg';
import image38 from './image38.jpg';
import image39 from './image39.jpg';
import image40 from './image40.jpg';
import image41 from './image41.jpg';
import image42 from './image42.jpg';
import image43 from './image43.jpg';
import image44 from './image44.jpg';
import image45 from './image45.jpg';
import image46 from './image46.jpg';
import image47 from './image47.jpg';
import image48 from './image48.jpg';
import image49 from './image49.jpg';
import image50 from './image50.jpg';
import image51 from './image51.jpg';
import image52 from './image52.jpg';
import image53 from './image53.jpg';
import image54 from './image54.jpg';
import image55 from './image55.jpg';
import image56 from './image56.jpg';
import image57 from './image57.jpg';
import exchange from './exchange.png';
import quality from './quality.png';
import customersupport from './customer-support.png';
import rightArrow from './right-arrow.png';
import closes from './close.png';
import star from './star.png';
import starnull from './starnull.png';
import image33a from './image33a.jpg';
import image30a from './image30a.jpg';
import image42a from './image42a.jpg';
import image34a from './image34a.jpg';
import image34b from './image34b.jpg';
import image34c from './image34c.jpg';
import image15a from './image15a.jpg';
import image17a from './image17a.jpg';
import image25a from './image25a.jpg';
import image27a from './image27a.jpg';
import image30b from './image30b.jpg';
import image30c from './image30c.jpg';
import image33b from './image33b.jpg';
import image33c from './image33c.jpg';
import image40a from './image40a.jpg';
import image46a from './image46a.jpg';
import image46b from './image46b.jpg';
import image48a from './image48a.jpg';
import image50a from './image50a.jpg';
import image54a from './image54a.jpg';
import image54b from './image54b.jpg';
import bin from './bin.png';
import stripe from './stripe.png';
import paypal from './paypal.png';
import aboutus from './aboutus.jpg';
import aboutus2 from './aboutus2.jpg';

export const assets = {
    logo,
    search,
    profile,
    shoppingcart,
    menu,
    back,
    hero,
    image1,
    image2,
    image3,
    image4,
    image5,
    image6,
    image7,
    image8,
    image9,
    image10,
    exchange,
    quality,
    customersupport,
    rightArrow,
    image11,
    image12,
    image14,
    image15,
    image16,
    image17,
    image18,
    image19,
    image20,
    image21,
    image22,
    image23,
    image24,
    image25,
    image26,
    image27,
    image28,
    image29,
    image30,
    image31,
    image32,
    image33,
    image34,
    image35,
    image36,
    image37,
    image38,
    image39,
    image40,
    image41,
    image42,
    image43,
    image44,
    image45,
    image46,
    image47,
    image48,
    image49,
    image50,
    image51,
    image52,
    image53,
    image54,
    image55,
    image56,
    image57,
    image33a,
    image33b,
    image30a,
    image42a,
    image34a,
    image34b,
    image34c,
    image15a,
    image17a,
    image25a,
    image27a,
    image30b,
    image30c,
    image40a,
    image46a,
    image46b,
    image48a,
    image50a,
    image54a,
    image54b,
    image33c,
    closes,
    star,
    starnull,
    bin,
    stripe,
    paypal,
    aboutus,
    aboutus2
} 

export const products = [
    {
        id:"DECO-001",
        name: "Modern Sofa",
        description: "Modern design meets ultimate comfort",
        price: 500,
        image:[image1],
        category: "Furniture",
        subCategory: "Living Room",
        colors: ["white","gray","blue"],
        date: 17022025,
        bestseller:true
    },
    {
        id:"DECO-002",
        name: "Wooden Vase",
        description: "A beautifully crafted wooden vase, combining natural elegance with timeless design.",
        price: 50,
        image:[image2],
        category: "Accessories",
        subCategory: "Home Decor",
        colors: ["brown","black"],
        date: 18022025,
        bestseller:true
    },
    {
        id:"DECO-003",
        name: "Nightstand",
        description: "Stylish and functional nightstands.",
        price: 280,
        image:[image3],
        category: "Furniture",
        subCategory: "Bedroom",
        colors: ["white","gray"],
        date: 19022025,
        bestseller:false
    },
    {
        id:"DECO-004",
        name: "Modern Kitchen",
        description: "Sleek and functional, a modern kitchen designed for style, efficiency, and seamless cooking experiences.",
        price: 3500,
        image:[image4],
        category: "Furniture",
        subCategory: "Kitchen",
        colors: ["white","blue","gray"],
        date: 21022025,
        bestseller:false
    },
    {
        id:"DECO-005",
        name: "Frame",
        description: "A stylish frame that enhances your photos and artwork, adding a touch of elegance and personality to any space.",
        price: 35,
        image:[image5],
        category: "Accessories",
        subCategory: "Home Decor",
        colors: ["white","black","gray"],
        date: 22022025,
        bestseller:false
    },
    {
        id:"DECO-006",
        name: "Cozy Lighting Lamp",
        description: "Elegant lamps that add warmth and style to any room, perfect for both functionality and décor.",
        price: 120,
        image:[image6],
        category: "Lighting",
        subCategory: "Home Decor",
        colors: ["blue","white"],
        date: 23022025,
        bestseller:false
    },
    {
        id:"DECO-007",
        name: "Unique vase",
        description: "A one-of-a-kind vase that adds character and charm to your home décor",
        price: 320,
        image:[image7],
        category: "Accessories",
        subCategory: "Home Decor",
        colors: ["brown"],
        date: 24022025,
        bestseller:false
    },
    {
        id:"DECO-008",
        name: "Woodland Seat",
        description:"A timeless wooden chair that combines comfort and natural elegance for any space.",
        price: 1000,
        image:[image8],
        category: "Furniture",
        subCategory: "Living Room",
        colors: ["brown"],
        date: 25022025,
        bestseller:true
    },
    
    {
        id:"DECO-009",
        name: "Reflecto Frame",
        description: "A beautifully crafted mirror frame that enhances your reflection. while adding a touch of sophistication to any room.",
        price: 490,
        image:[image9],
        category: "Decorative Art",
        subCategory: "Mirrors",
        colors: ["brown"],
        date: 26022025,
        bestseller:true
    },
    {
        id:"DECO-010",
        name: "Cozy and simple dinning table",
        description: "A simple yet incredibly cozy chair designed for ultimate comfort and relaxation in any space.",
        price: 895,
        image:[image10],
        category: "Furniture",
        subCategory: "Dining Room",
        colors: ["white","gray","black"],
        date: 27022025,
        bestseller:false
    },
    {
        id:"DECO-011",
        name: "Glass vase",
        description: "A transparent container for flowers or decoration.",
        price: 120,
        image:[image11],
        category: "Accessories",
        subCategory: "Home Decor",
        colors: ["black","dark blue","yellow"],
        date: 28022025,
        bestseller:true
    },
    {
        id:"DECO-012",
        name: "Comfy Sofa",
        description: "A cozy, cushioned seat for relaxation.",
        price: 550,
        image:[image12],
        category: "Furniture",
        subCategory: "Living Room",
        colors: ["brown","black"],
        date: 1032025,
        bestseller:false
    },
    {
        id:"DECO-014",
        name: "White vase",
        description: "A sleek, elegant container for flowers or decor.",
        price: 400,
        image:[image14],
        category: "Accessories",
        subCategory: "Home Decor",
        colors: ["white","black"],
        date: 2032025,
        bestseller:false
    },
    {
        id:"DECO-015",
        name: "Wooden chair",
        description: "A sturdy, classic seat made of wood.",
        price: 570,
        image:[image15,image15a],
        category: "Furniture",
        subCategory: "Living Room",
        colors: ["brown"],
        date: 3032025,
        bestseller:false
    },
    {
        id:"DECO-016",
        name: "Modern chair",
        description: "A stylish, ergonomic seat with a sleek design.",
        price: 420,
        image:[image16],
        category: "Furniture",
        subCategory: "Living Room",
        colors: ["black","grat"],
        date: 4032025,
        bestseller:false
    },
    {
        id:"DECO-017",
        name: "Wooden Frame and white vase",
        description: "A clean, elegant frame and a stylish white vase for decor.",
        price: 240,
        image:[image17,image17a],
        category: "Accessories",
        subCategory: "Home Decor",
        colors: ["white","black"],
        date: 5032025,
        bestseller:false
    },
    {
        id:"DECO-018",
        name: "Special texture chair",
        description:"A sturdy wooden chair featuring a unique, textured finish.",
        price: 900,
        image:[image18],
        category: "Furniture",
        subCategory: "Living Room",
        colors: ["brown"],
        date: 6032025,
        bestseller:false
    },
    
    {
        id:"DECO-019",
        name: "Colorfull pillows",
        description: "Vibrant, soft cushions adding warmth and style.",
        price: 80,
        image:[image19],
        category: "Accessories",
        subCategory: "Home Decor",
        colors: ["blue and white","pink and white","green and white","yellow and white"],
        date: 7032025,
        bestseller:false
    },
    {
        id:"DECO-020",
        name: "Custom-shaped nightstands",
        description: "Unique nightstands with a distinctive, eye-catching design.",
        price: 299,
        image:[image20],
        category: "Furniture",
        subCategory: "Beedroom",
        colors: ["white","blue"],
        date: 8032025,
        bestseller:false
    },
    {
        id:"DECO-021",
        name: "Simply wooden frame",
        description: "A basic, natural wooden frame with a minimalist design.",
        price: 70,
        image:[image21],
        category: "Accessories",
        subCategory: "Home Decor",
        colors: ["white"],
        date: 9032025,
        bestseller:false
    },
    {
        id:"DECO-022",
        name: "Decorative shelf",
        description: "AA simple shelf designed to hold vases, bottles, and decorative items.",
        price: 320,
        image:[image22],
        category: "Furniture",
        subCategory: "Living Room",
        colors: ["brown"],
        date: 10032025,
        bestseller:false
    },
    {
        id:"DECO-023",
        name: "Special wooden table",
        description: "SA unique, handcrafted wooden table with a distinctive design or texture.",
        price: 1720,
        image:[image23],
        category: "Furniture",
        subCategory: "Bedroom",
        colors: ["black","gray"],
        date: 11032025,
        bestseller:false
    },
    {
        id:"DECO-024",
        name: "Simply desk",
        description: "A basic, functional desk designed for work.",
        price: 820,
        image:[image24],
        category: "Furniture",
        subCategory: "Bedroom",
        colors: ["white","black"],
        date: 12032025,
        bestseller:false
    },
    {
        id:"DECO-025",
        name: "Unique form vase",
        description: "A vase with a distinctive, artistic shape that stands out as a decorative piece.",
        price: 650,
        image:[image25,image25a],
        category: "Accessories",
        subCategory: "Home Decor",
        colors: ["white"],
        date: 13032025,
        bestseller:false
    },
    {
        id:"DECO-026",
        name: "Unique dining table",
        description: "A large table designed for meals, with a special material.",
        price: 5200,
        image:[image26],
        category: "Furniture",
        subCategory: "Dinning Room",
        colors: ["gray"],
        date: 14032025,
        bestseller:false
    },
    {
        id:"DECO-027",
        name: "Dinning table",
        description: "A small table with a special design.",
        price: 3200,
        image:[image27,image27a],
        category: "Furniture",
        subCategory: "Dinning Room",
        colors: ["brown"],
        date: 15032025,
        bestseller:false
    },
    {
        id:"DECO-028",
        name: "Special design kitchen",
        description:"Kitchen with special design and color.",
        price: 4100,
        image:[image28],
        category: "Furniture",
        subCategory: "Kitchen",
        colors: ["green","blue","red"],
        date: 16032025,
        bestseller:false
    },
    
    {
        id:"DECO-029",
        name: "Chair set",
        description: "A collection of matching chairs, typically designed to complement a specific table or setting.",
        price: 890,
        image:[image29],
        category: "Furniture",
        subCategory: "Living Room",
        colors: ["yellow"],
        date: 17032025,
        bestseller:false
    },
    {
        id:"DECO-030",
        name: "Wood chair",
        description: "A uniquely designed wooden chair, crafted with attention to detail and featuring distinctive textures or shapes.",
        price: 730,
        image:[image30,image30a,image30b,image30c],
        category: "Furniture",
        subCategory: "Dinning Room",
        colors: ["brown"],
        date: 18032025,
        bestseller:false
    },
    {
        id:"DECO-031",
        name: "Towel holder",
        description: "A fixture used to hang and organize towels.",
        price: 50,
        image:[image31],
        category: "Accessories",
        subCategory: "Home Decor",
        colors: ["brown"],
        date: 19032025,
        bestseller:false
    },
    {
        id:"DECO-032",
        name: "Toothbrush holder",
        description: "A container used to store and organize toothbrushes.",
        price: 20,
        image:[image32],
        category: "Accessories",
        subCategory: "Home Decor",
        colors: ["cream","white"],
        date: 20032025,
        bestseller:false
    },
    {
        id:"DECO-033",
        name: "Unique form vase",
        description: "A vase with a distinctive, artistic shape that serves as both a functional container and a decorative piece, often showcasing creativity and craftsmanship.",
        price: 310,
        image:[image33,image33a,image33b,image33c],
        category: "Accessories",
        subCategory: "Home Decor",
        colors: ["white","creme"],
        date: 21032025,
        bestseller:false
    },
    {
        id:"DECO-034",
        name: "Special chair",
        description: "Very special chair.",
        price: 800,
        image:[image34,image34a,image34b,image34c],
        category: "Furniture",
        subCategory: "Living Room",
        colors: ["brown"],
        date: 22032025,
        bestseller:false
    },
    {
        id:"DECO-035",
        name: "Vintage couch",
        description: "A vintage-style couch with a classic, timeworn design, often featuring intricate details and traditional upholstery.",
        price: 4500,
        image:[image35],
        category: "Furniture",
        subCategory: "Living Room",
        colors: ["brown"],
        date: 23032025,
        bestseller:false
    },
    {
        id:"DECO-036",
        name: "Small and cozy kitchen",
        description: "A compact and inviting kitchen, designed for efficiency and comfort, with a warm, homely atmosphere.",
        price: 2500,
        image:[image36],
        category: "Furniture",
        subCategory: "Kitchen",
        colors: ["white","blue"],
        date: 24032025,
        bestseller:false
    },
    {
        id:"DECO-037",
        name: "Dark blue chair",
        description: "A chair with a vibrant blue color, adding a pop of color and style to any space.",
        price: 450,
        image:[image37],
        category: "Furniture",
        subCategory: "Dinning Room",
        colors: ["dark blue"],
        date: 25032025,
        bestseller:false
    },
    {
        id:"DECO-038",
        name: "Tea Cup Holder",
        description: "A tea cup holder is a holder specifically designed to store or display tea cups, keeping them organized and easily accessible.",
        price: 120,
        image:[image38],
        category: "Accessories",
        subCategory: "Home Decor",
        colors: ["white"],
        date: 25032025,
        bestseller:false
    },
    {
        id:"DECO-039",
        name: "Modern Kitchen",
        description:"A sleek, stylish kitchen with contemporary appliances, clean lines, and minimalist design, often featuring open spaces and modern materials.",
        price: 2500,
        image:[image39],
        category: "Furniture",
        subCategory: "Kitchen",
        colors: ["white"],
        date: 26032025,
        bestseller:false
    },
    {
        id:"DECO-040",
        name: "Cozy couch",
        description: "A simple and cozy couch.",
        price: 600,
        image:[image40,image40a],
        category: "Furniture",
        subCategory: "Living Room",
        colors: ["creme","gray","white"],
        date: 28032025,
        bestseller:false
    },
    {
        id:"DECO-041",
        name: "Cute chair",
        description: "A cute chair with pink color.",
        price: 570,
        image:[image41],
        category: "Furniture",
        subCategory: "Living Room",
        colors: ["pink"],
        date: 29032025,
        bestseller:false
    },
    {
        id:"DECO-042",
        name: "Side table",
        description: "A small table used for placing decor items like vases and lamps.",
        price: 150,
        image:[image42,image42a],
        category: "Furniture",
        subCategory: "Living Room",
        colors: ["black"],
        date: 30032025,
        bestseller:false
    },
    {
        id:"DECO-043",
        name: "Simple chair",
        description: "Simple yet perfect chair.",
        price: 250,
        image:[image43],
        category: "Furniture",
        subCategory: "Dinning Room",
        colors: ["white","black"],
        date: 31032025,
        bestseller:false
    },
    {
        id:"DECO-044",
        name: "Special pictures",
        description: "A simple yet unique picture, featuring a clean design or striking subject, adding a touch of elegance and personality to any space.",
        price:  70,
        image:[image44],
        category: "Decorative Art",
        subCategory: "Art",
        colors: ["white","black","gray"],
        date: 1042025,
        bestseller:false
    },
    {
        id:"DECO-045",
        name: "Table with two-chairs",
        description: "A unique and small table.",
        price: 350,
        image:[image45],
        category: "Furniture",
        subCategory: "Dinning Room",
        colors: ["brown"],
        date: 2042025,
        bestseller:false
    },
    {
        id:"DECO-046",
        name: "Side black table",
        description: "Simple black side table for your bedroom.",
        price: 140,
        image:[image46,image46a,image46b],
        category: "Furniture",
        subCategory: "Bedroom",
        colors: ["black"],
        date: 3042025,
        bestseller:false
    },
    {
        id:"DECO-047",
        name: "Special shaped mirror",
        description: "Very special shaped mirror that bring amazing decor.",
        price: 170,
        image:[image47],
        category: "Decorative Art",
        subCategory: "Mirrors",
        colors: ["transparent"],
        date: 5042025,
        bestseller:false
    },
    {
        id:"DECO-048",
        name: "Modern and classic sofa",
        description:"A sofa that blends modern design with classic elements, offering both contemporary style and timeless elegance.",
        price: 1900,
        image:[image48,image48a],
        category: "Furniture",
        subCategory: "Living Room",
        colors: ["white"],
        date: 6042025,
        bestseller:false
    },
    
    {
        id:"DECO-049",
        name: "Special chair",
        description: "A special wood chair to read books and drink you coffee or tea.",
        price: 490,
        image:[image49],
        category: "Furniture",
        subCategory: "Living Room",
        colors: ["white"],
        date: 7042025,
        bestseller:false
    },
    {
        id:"DECO-050",
        name: "Black chair",
        description: "A simple black chair that add beautiful design to your dinning room.",
        price: 315,
        image:[image50,image50a],
        category: "Furniture",
        subCategory: "Dining Room",
        colors: ["black"],
        date: 8042025,
        bestseller:false
    },
    {
        id:"DECO-051",
        name: "Special sculpture",
        description: "An amazing touch for your home decor.",
        price: 120,
        image:[image51],
        category: "Decorative Art",
        subCategory: "Art",
        colors: ["black"],
        date: 9042025,
        bestseller:false
    },
    {
        id:"DECO-052",
        name: "Lamp",
        description: "A special lamp for your home.",
        price: 65,
        image:[image52],
        category: "Lighting",
        subCategory: "Home Decor",
        colors: ["white"],
        date: 10042025,
        bestseller:false
    },
    {
        id:"DECO-053",
        name: "Square painting",
        description: "Stylish square painting.",
        price: 80,
        image:[image53],
        category: "Decorative Art",
        subCategory: "Art",
        colors: ["brown"],
        date: 11042025,
        bestseller:false
    },
    {
        id:"DECO-054",
        name: "Simple white chair",
        description: "Simple chair for your dinning room.",
        price: 70,
        image:[image54,image54a,image54b],
        category: "Furniture",
        subCategory: "Dinning Room",
        colors: ["white"],
        date: 12042025,
        bestseller:false
    },
    {
        id:"DECO-055",
        name: "Ceiling lights",
        description: "Amazing ceiling lights with a special design.",
        price: 450,
        image:[image55],
        category: "Lighting",
        subCategory: "Home Decor",
        colors: ["white","black","gray"],
        date: 13042025,
        bestseller:false
    },
    {
        id:"DECO-056",
        name: "Sculpture",
        description: "A very special sculpture to add art in your home desing.",
        price: 3220,
        image:[image56],
        category: "Decorative Art",
        subCategory: "Art",
        colors: ["white"],
        date: 14042025,
        bestseller:false
    },
    {
        id:"DECO-057",
        name: "Special Sculpture",
        description: "Special sculpture with special color.",
        price: 8250,
        image:[image57],
        category: "Decorative Art",
        subCategory: "Art",
        colors: ["gray"],
        date: 15042025,
        bestseller:false
    },
   
]