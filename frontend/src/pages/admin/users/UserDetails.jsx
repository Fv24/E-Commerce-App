...
useEffect(() => {
  fetchRoles(); // Call fetchRoles to get the roles when the component mounts
}, [token]);
  ...
  const handleDeleteRole = async (roleId) => {
    try {
      ...
      if (response.status === 200) {
        setRoles((prevRoles) => prevRoles.filter((role) => role.id !== roleId)); // Update roles state with the filtered list
...
const handleCreateRole = async () => {
  try {
    ...
    if (response.status === 200) {
      ...
      const createdRole = response.data; // Get the created role from response
      setRoles((prevRoles) => [...prevRoles, { id: createdRole.id, name: newRoleName }]); // response.data should contain the role with its name and other details
      setNewRoleName(""); // Clear the input field for new role name
